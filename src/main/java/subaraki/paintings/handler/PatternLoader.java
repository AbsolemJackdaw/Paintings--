...
public class PatternLoader {
	...
	public void loadPatterns() {
		...
		if(resURL == null) {
			MorePaintings.log.warn("Unable to access file %s: got 'null' when trying to resolve it", base);
			return;
		}
		...
		if(in == null)
		{
			MorePaintings.log.warn("Coult not read painting data. paintings will be unusable");
			return;
		}
		BufferedReader reader = new BufferedReader(new InputStreamReader(in));
		...
		PaintingPattern.instance = gson.fromJson(json, PaintingPattern.class);
		PaintingPattern.instance.loadPatterns();
	}
}
